...
[... Content truncated for brevity - keeping existing entries ...]
...
[x] 673. MOBILE FUNDAMENTAL ANALYSIS NEWS TOGGLE - November 18, 2025 06:15 AM
[x] 674. User requested: Add news icon switch in top-right of Fundamental Analysis for mobile screens
[x] 675. Added Newspaper icon import from lucide-react
[x] 676. Added showNewsOnMobile state to toggle between Fundamentals and News views
[x] 677. Added newspaper icon button in Fundamental Analysis header (visible only on mobile with lg:hidden)
[x] 678. Dynamic header title changes: "Fundamental Analysis" ↔ "Related News" based on toggle state
[x] 679. Conditional content rendering on mobile - shows either Fundamentals or News (not both)
[x] 680. Related News content duplicated inside Fundamental Analysis panel for mobile toggle
[x] 681. Separate Related News panel now hidden on mobile (hidden lg:block) - visible only on desktop
[x] 682. Mobile users can now tap newspaper icon to switch between views without scrolling
[x] 683. Desktop/tablet (lg+) maintains side-by-side layout - no changes to existing behavior
